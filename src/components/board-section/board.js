...
import { AttackDirection, AttackType, DIRECTION_TIER_MAP, } from "../../types/character";
...
export const BOARD_SIZE = GRID_SIZE * GRIDS_IN_LINE + GAP * (GRIDS_IN_LINE - 1) + PADDING * 2;
export class Board extends GameObjectClass {
    constructor(x, y) {
        ...
        Object.defineProperty(this, "occuInfo", {
            enumerable: true,
            configurable: true,
            writable: true,
        ...
        for (let i = moveUp || moveDown ? 0 : startI; i !== endI; i += stepI) {
            for (let j = moveLeft || moveRight ? 0 : startJ; j !== endJ; j += stepJ) {
                ...
                while (true) {
                    ...
                    const occupiedCard = this.occuInfo?.[nextJ]?.[nextI];
                    if (occupiedCard) {
                        const isTemplarEquip = card.type === CardType.T && occupiedCard instanceof ItemCard;
                        const isItemUpgrade = card.type === occupiedCard.type && card instanceof ItemCard;
    ...
    async checkAttack(direction) {
        ...
        for (const { attacker, target, direction } of battleInfos) {
            await attacker.execAttack(direction, target, false, attacker.attackType === AttackType.P);
        }
    }
    async checkDuration() {
        const gm = GameManager.gI();
                ...
                Object.entries(item.buff).forEach(([key, value]) => {
                    ...
                    if (key === ad) {
                        const remain = gm.currentItems
                            ...
                            .sort((a, b) => DIRECTION_TIER_MAP[a.buff[ad]] -
                            DIRECTION_TIER_MAP[b.buff[ad]])[0];
                        debuff[key] = remain ? remain.buff[ad] : AttackDirection.F;
                    }
                    else if (key === at) {
                        const remain = gm.currentItems.filter((i) => !!i.buff[at] && i.duration > 0)[0];
